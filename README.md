...
This library relies on `react-native-reanimated`, `react-native-gesture-handler`, and `react-native-nitro-modules` for JSI/C++ speed. Install them in your project:

```bash
npm install react-native-reanimated react-native-gesture-handler react-native-nitro-modules
```
...
**rn-smart-sheet** uses **Margelo's Nitro Modules** under the hood. This offers a compile-time generated JSI and C++ binding layer that makes interactions between JS and Native virtually instantaneous.

### How it works:
- **JSI-direct bindings**: By using JSI, we bypass the legacy React Native bridge. Methods like `snapToIndex`, `snapToPosition`, and `close` are direct JSI calls executing on the native UI thread.
- **Fabric/Bridgeless Ready**: Fully optimized for the New Architecture, utilizing C++ templates generated by Nitrogen.
- **Autolinking**: Both Android and iOS autolink out-of-the-box. On Android, CMake compiles the generated C++ glue library, while on iOS, Cocoapods integrates the Swift-C++ bridges automatically.
